...
vector[I] delta2;                   // DIF parameter for item k
...
for (n in 1:N)
eta[n] = (alpha[ii[n]]+ delta1[ii[n]] * x[jj[n]]) * (theta[jj[n]] - (beta[ii[n]] + delta2[ii[n]] * x[jj[n]]));
y ~ bernoulli_logit(eta);
}
"


sim_fit <- stan(model_code=mod.stan, pars=c("alpha","beta","gamma","delta1","delta2"),
                data = irt.data, chains = 2, iter = 500)
sim_fit
