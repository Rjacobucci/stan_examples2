...
matrix[N,4] sigma; // residual score matrix
...
model{
  ...
  for(i in 1:N){    
      for(j in 1:4){
        X[i,j] ~ normal(mu[i,j],.000001); // fix variance to .0000001
        sigma[i,j] ~ normal(M2[j],1);
      }
