...
vector[4] sig;
//real alpha;
matrix[N,1] init_sigma;
matrix[N,3] sigma; // residual score matrix
}
...
for(i in 1:N){
mu[i,1] = FS[i,1] + 1*FS[i,2] + init_sigma[i,1];
mu[i,2] = FS[i,1] + 2*FS[i,2] + sigma[i,1]; 
mu[i,3] = FS[i,1] + 4*FS[i,2] + sigma[i,2];
mu[i,4] = FS[i,1] + 6*FS[i,2] + sigma[i,3];
...
for(i in 1:N){    
for(j in 1:4){
X[i,j] ~ normal(mu[i,j],.0000001); // fix variance to .0000001
}
init_sigma[i,1] ~ normal(0,sig[1]);
for(j in 1:3){
sigma[i,j] ~ normal(0,sig[j+1]);
}
